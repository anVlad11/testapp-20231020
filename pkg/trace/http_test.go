...
func TestErrorOnlyHandledOnce(t *testing.T) {
	...
	router.Use(MakeTracingMiddleware("test-service", "test", "v0.0.1"))
...
func TestPropagationWithGlobalPropagators(t *testing.T) {
	...
	router := echo.New()
	router.Use(MakeTracingMiddleware("foobar", "test", "v0.0.1", WithTracerProvider(provider)))
...
func TestSkipper(t *testing.T) {
	...
	router := echo.New()
	router.Use(MakeTracingMiddleware("foobar", "test", "v0.0.1", WithSkipper(skipper)))
	router.GET("/ping", func(c echo.Context) error {
		span := trace.SpanFromContext(c.Request().Context())
